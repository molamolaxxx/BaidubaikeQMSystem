...
class Saver():
    ...
    def connect_mysql(self):
        '''连接数据库'''
        db=pymysql.connect(host = SpiderConfig.host, port = SpiderConfig.port, user = SpiderConfig.username, passwd = SpiderConfig.password, db = SpiderConfig.db_name)
        return db
    ...
    def save_one_item(self,item):
        ...
        if item.is_available==True:
            ...
            print("存储到mysql")
            #创建游标
            cursor=self.database.cursor(cursor = pymysql.cursors.DictCursor)
